...
module ActiveMerchant
  module Shipping
    class UPS < Carrier
      ...
      DEFAULT_SERVICE_NAME_TO_CODE = Hash[DEFAULT_SERVICES.to_a.map(&:reverse)]
      ...
      def create_shipment(origin, destination, packages, options = {})
        ...
        raise ActiveMerchant::Shipping::ResponseContentError, StandardError.new(message) unless success
      ...
      def build_document(xml, expected_root_tag)
        ...
        if document.root.nil? || document.root.name != expected_root_tag
          raise ActiveMerchant::Shipping::ResponseContentError.new(StandardError.new('Invalid document'), xml)
        end
        document
      ...
      def commit(action, request, test = false)
        ...
        response = ssl_post("#{url}/#{RESOURCES[action]}", request)
